...
namespace weatherforecast.Tests
{
    ...
    {
        [TestMethod()]
        public void CheckValidTest_ValidData_ReturnsTrue()
        {
            // 1. Створення Borders (0-4 — TextBox, 5-6 — ComboBox)
            ...
            // 2. Заповнення TextBox-ів валідними значеннями
            ...
            // 3. Виклик методу
            bool isValid = ValidationService.CheckValid(out string error, borders);

            // 4. Перевірка відповідності очікуваних і фактичних значень
        ...
        [TestMethod()]
        public void CheckValidTest_EmptyData_ReturnsFalse()
        {
            // 1. Створення Borders (0-4 — TextBox, 5-6 — ComboBox)
            Border[] borders = new Border[7];
            for (int i = 0; i < 5; i++)
            ...
            // 2. Заповнення не всіх TextBox-ів
            ((TextBox)borders[0].Child).Text = "25";    // Температура
        ...
        [TestMethod()]
        public void CheckValidTest_InvalidDataRange_ReturnsFalse()
        {
            // 1. Створення Borders (0-4 — TextBox, 5-6 — ComboBox)
            Border[] borders = new Border[7];
            for (int i = 0; i < 5; i++)
            ...
            // 2. Заповнення TextBox-ів значеннями, що виходять за межі допустимих діапазонів
            ((TextBox)borders[0].Child).Text = "25";    // Температура
        ...
        [TestMethod()]
        public void CheckValidTest_InvalidDataFormat_ReturnsFalse()
        {
            // 1. Створення Borders (0-4 — TextBox, 5-6 — ComboBox)
            Border[] borders = new Border[7];
            for (int i = 0; i < 5; i++)
            ...
            // 2. Заповнення TextBox-ів даними неправильного формату
            ((TextBox)borders[0].Child).Text = "25";    // Температура
